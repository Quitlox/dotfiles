...
local function is_neotest_buf(bufnr)
    ...
    return status and ft == "neotest-summary"
end
...
M.on_save = function()
    ...
    -- Close neotest summary if it's open
    if is_open then
        pcall(require("neotest").summary.close)
    end

    -- Return whether neotest was open
...
M.on_post_load = function(data)
    ...
    -- If neotest was open when the session was saved, reopen it
    if data and data.is_open then
        pcall(require("neotest").summary.open)
    end
end
