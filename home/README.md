...
Chezmoi is a dotfiles manager. It uses a _source state_ (in
`~/.local/share/chezmoi`) and a _target state_. The _source state_ is committed
to version control. When running `chezmoi apply`, the _source state_ is used to
produce the _target state_.
